...
const List = () => {
  ...
  const { data, isLoading } = useFetch(
    "https://students-manager.onrender.com/getStudentsData"
  );
  ...
  const deleteAllStudents = async () => {
    ...
    const res = await fetch(
      `https://students-manager.onrender.com/deleteAllStudents/`,
      {
        method: "DELETE",
        headers: {
  ...
  const deleteStudent = async (rollNumber: any) => {
    setShow(false);
    const res = await fetch(
      `https://students-manager.onrender.com/deleteStudent/${rollNumber}`,
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
  ...
  const sortData = (data: any[], rollNumber: any) => {
    const sorted = [...data].sort(
      (a: any, b: any) => a[rollNumber] - b[rollNumber]
    );
    ...
    <div className={classes.listContainer}>
            ...
            <Btn
              label={
                <div className={classes.btnLabel}>
                  ...
                  <span className={classes.delIcon}>
                    <IoPersonRemove />
                  </span>
                </div>
              }
          ...
          {sortedData
            .filter((val: any) => {
              if (searchText === "") {
                ...
                val.rollNumber === searchText
              ) {
                return val;
              }
            })
            .map((el: any, index: any) => {
              const { name, rollNumber, attendance, marks } = el;
              return (
                <div key={index}>
                  <Card
                    //   styleOverride={
                    ...
                    marks={marks}
                    onDelete={() => {
                      showDeletionModal(rollNumber);
                    }}
                    onEdit={() => {
                      navigate(`/edit/${rollNumber}`);
                    }}
                  />
                </div>
              );
      ...
      {open && (
        <Modal
          ...
          modalContent={modalContentDeleteAll}
        ></Modal>
      )}
    </div>
  );
